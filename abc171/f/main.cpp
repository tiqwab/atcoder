...
int main(void) {
    ...
    Combination *c = new Combination(2 * MAX_N, MOD);

    mint ans = 0;

    for (int i = slen; i <= slen + K; i++) {
        // i is a position for S[slen - 1]
        mint acc = 1;
        acc *= mint(25).pow(i - slen);
        acc *= c->com(i - 1, slen - 1); 
        acc *= mint(26).pow(slen + K - i);
        ans += acc;
    }
