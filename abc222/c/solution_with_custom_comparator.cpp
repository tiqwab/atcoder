...
// we can't use lambda?
// it causes a compile error at swap
bool comp(const pair<int, int> &one, const pair<int, int> &two) {
...
int main(void) {
    ...
    // { wins, id }
    set<pair<int, int>, decltype(&comp)> scores(&comp);
    for (int i = 0; i < 2 * N; i++) {
        scores.insert(make_pair(0, i));
    }

    for (int m = 0; m < M; m++) {
        set<pair<int, int>, decltype(&comp)> tmp(&comp);
